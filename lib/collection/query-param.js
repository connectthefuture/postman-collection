...
_.assign(QueryParam, /** @lends QueryParam */ {
    ...
    unparse: function (params, options) {
        ...
        var count = params.length - 1,
            ...
            str = params.reduce(function (result, param, index) {
                var isNextEnabled;

                // check if the next query param exists and is enabled, overriding with options.ignoreDisabled
                if (index < count) {
                    isNextEnabled = !(ignoreDisabled && _.get(params[index + 1], 'disabled'));
                }

                // If disabled parameters are to be ignored, bail out here
                if (ignoreDisabled && param.disabled) {
                    // Append an `&` to the result if both of the following conditions are satisfied:
                    // 1. The next query parameter in the list exists and is not disabled.
                    // 2. The current result is not empty.
                    // The 2nd condition prevents consecutive disabled query params from being converted to a sequence
                    // of ampersands. See the unit tests for more context.
                    result && isNextEnabled && (result += AMPERSAND);

                    return result; // since the current query param is disabled, bail out without attempting to unparse
                }

                result += QueryParam.unparseSingle(param, encode); // Add the unparsed current query param to the result

                // Append an ampersand only if the next query param exists and is not disabled
                isNextEnabled && (result += AMPERSAND);
